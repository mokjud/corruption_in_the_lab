...
datafile = "C:/Users/fedor/OneDrive/Documents/DOKUMENTUMOK/Corruption.Project/GitHub/power_simulations/dummydata_20190404-101536.RData"
...
These are based on Weisel & Shalvi, 2015, Fig. S10.  
One plot per participant.
Circles represent the simulated values of "player A", plus signs represent the reported numbers of player B.
...
for(i in person[1]){
  ...
  figure1 <- ggplot(data=filter(rawdata, ID==i), aes(x=Index))+
    ...
    theme_classic()+
    scale_x_continuous(name="Period of game")+
    scale_y_continuous(name="Reported value")+
    ggtitle(paste("Participant ID:", i))
...
for (i in groups$Condition) {
  ...
  w1[[i]] <- wilcox.test(x=current$Nbof_doubles, mu=parameters$nbof_trials[nrow(parameters)]/parameters$dice_sides[nrow(parameters)], alternative = "greater")
}

result.wilcox.table1 <- data.frame(
...
kable(result.wilcox.table1, digits=10)
