...
df.results <- data.frame(condition=cond, D=NA, p_value=NA)
for(i in cond)
{
  group_data <- filter(rawdata, Condition == i)
  result <- ks.test(group_data$ValueB, honestvalues_cdf, alternative = "two.sided")
  #cat("  \n Condition:",  paste(i), "  \n") 
  #print(result)
  df.results[which(df.results$condition==i), ]$D <- result$statistic
  df.results[which(df.results$condition==i), ]$p_value <- result$p.value
