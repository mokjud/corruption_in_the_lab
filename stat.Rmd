...
N = 20 # number of participants in the dummy data
...
person <- person[1] #to try the figure ww will print only the first five subjects' reports. 
...
```{r Generate useful variables from raw data}
...
Our null hípothesis was that the reported values come from a uniform distribution, i.e. numbers from 1 to 6 are reported with the same probability, since we used a fair dice. Our alternative hypothesis is that participants cheat and report doubles in order to inflate their profit.  

This would lead to skewed distributions when the participant playes with a dishonest simulated partner, who reports higher numbers with higher probability. In case of a honest simulated partner, whose dice throw values were sampled from a uniform distribution, we expect that the participants' reported values also come from a uniform ditribution (although it is possible that participants try to signal to their supposed partners to cheat by occassionally reporting 6s, but we do not expect this create a significant difference).
...
* Or two-sided everywhere, because we think this is what others did
...
ks.test(group_SH$ValueB, honestvalues_cdf, alternative = "two.sided")
ks.test(group_SD$ValueB, honestvalues_cdf, alternative = "two.sided")
ks.test(group_CH$ValueB, honestvalues_cdf, alternative = "two.sided")
ks.test(group_CD$ValueB, honestvalues_cdf, alternative = "two.sided")
...
qqplot(group_SH$ValueB, honestvalues)
qqplot(group_SD$ValueB, honestvalues)
qqplot(group_CH$ValueB, honestvalues)
qqplot(group_CD$ValueB, honestvalues)
