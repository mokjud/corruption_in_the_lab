...
## create four data.frame for the four game types
  ...
  # loop for sorting the elements of the list into the right dataframe. 
    ...
    #View(dishonest_charity)
    #View(dishonest_nocharity)
    #View(honest_charity)
    #View(honest_nocharity)
    
    
## create datasets that looks like this
  # we need a dataset that looks like this: 
    ...
    #dishonest_charity_rawdata <- data.frame(Time=dishonest_charity[,1], ID=NA, Condition=NA, Game=NA, Partner=NA, Index=NA, ValueA=NA, ValueB=NA, Double=NA)
...
## save the dataframe into a csv
  write.csv(fulldata_rawdata, file = "/Users/mokosjudit/Google Drive/Korrupció_a_laborban_ötletek/eles_nevaltoztassrajta/csv/template_datasets/data_ztree.csv")
