...
# must do by hand: convert all the original ztree files to csv. the file created by ztree is not a normal xls, therefore R can't use it. 
...
datadir <- "C:/Users/fedor/Desktop/eles_nevaltoztassrajta/csv/" # PUT ALL CSV FILES INTO THIS FOLDER!
...
#myfiles by defoult is factor. this loop makes everything character. 
...
## create four data.frames for the four game types

#creating 4 empty dataframes to fill up
dishonest_charity <- read.csv(paste(datadir, "template_datasets/proba_dishonest_charity.csv", sep=""), header = T)
...
#Create dataframe for stat.Rmd to read
...
## games that was played by the experimenters has the ID number of 1000. These rows need to be deleted. 

fulldata_rawdata <- filter(fulldata_rawdata, ID!=1000) #exclude trial cases. THIS DATASET IS THE ONE THAT USED IN THE ANALYSES. 
fulldata_rawdata <- filter(fulldata_rawdata, ID!=999) #exclude trial cases. THIS DATASET IS THE ONE THAT USED IN THE ANALYSES. 
...
write.csv(fulldata_rawdata, file = paste(datadir, "/template_datasets/data_ztree.csv", sep=""))
...
#delete ID=9250 because the game was bugging and gave -1 for the 13th round instead of a normal number. 
fulldata_rawdata <- filter(fulldata_rawdata, ID!=9250)
